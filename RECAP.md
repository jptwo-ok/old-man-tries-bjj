...
## Task
Two more bugs in the expanded grid tile's video behavior, following up on RECAP 17's fullscreen button. Bug 1: exiting native fullscreen collapsed the tile back to the plain grid thumbnail and stopped playback, instead of returning to the expanded-in-grid overlay with playback continuing unmuted. Bug 2: swiping to an adjacent clip while expanded could autoplay it muted, unlike tap-to-expand which reliably autoplays with sound. Talked through the plan first (an exploratory "how does this sound?" pass, not yet an implementation request) — flagged that Bug 1's originally-suggested fix (skip a `fullscreenchange`-triggered collapse) had no actual collapse-on-exit code to skip, since no fullscreen-exit listener existed at all before this change, and the real cause is more likely a stray click some browsers pass through to the page once the native fullscreen-exit control closes; and that Bug 2 already runs through one shared autoplay effect for both tap and swipe (no separate "muted init" path to redirect), so the more likely cause is mobile autoplay-permission timing rather than divergent code paths. The user agreed and asked for a stray-tap guard (ref-based, mirroring `longPressFired.current`) for Bug 1 instead of a `fullscreenchange` listener premised on removing non-existent logic, and an earlier `play()` call closer to the swipe gesture for Bug 2, keeping the video.play()-if-paused-on-exit part of the original plan either way.

## What was done

**Bug 1** ([components/ClipGrid.jsx](components/ClipGrid.jsx)): added `ignoreNextTapRef`, a ref alongside the existing touch-gesture refs (`longPressFired`, `movedRef`, etc.), plus a `useEffect` (keyed on `isExpanded`) that listens for `fullscreenchange` on `document` (checking `!document.fullscreenElement` to detect exit) and `webkitendfullscreen` directly on the video element (iOS Safari's native video fullscreen has no `requestFullscreen`/`fullscreenchange` at all). On exit, it sets `ignoreNextTapRef.current = true` — consumed and cleared by `handleClick` and `handleTouchEnd`'s genuine-tap branch, both of which now return early without toggling `expandedId` when the flag is set — and calls `video.play()` if the browser left it paused. The flag also self-clears via `setTimeout` after 500ms (matching the order of magnitude of this file's existing `LONG_PRESS_MS`) so an exit path that never actually produces a stray click (e.g. desktop Escape key) doesn't end up swallowing the user's next real, deliberate tap.

**Bug 2** (`components/ClipGrid.jsx`): the existing autoplay-with-sound effect was already shared between tap-to-expand and swipe/arrow-key/chevron navigation (there was no separate muted-initialization branch to find and fix). Switched it from `useEffect` to a new `useIsomorphicLayoutEffect` (`typeof window !== "undefined" ? useLayoutEffect : useEffect`, falling back during SSR since `ClipGrid` is a `"use client"` component that still gets server-rendered for the initial HTML, and `useLayoutEffect` warns in that context) — `useLayoutEffect` fires synchronously right after the newly-expanded tile's `<video>` mounts, before the browser paints, rather than being deferred past paint like a normal `useEffect`, minimizing the delay between the original gesture (tap or swipe) and the `play()` call. Verified the build produces no SSR warnings from this.

## Verification
`npm run build` passed cleanly — all 18 routes generated, no errors, no SSR warnings.
...
## Task
Investigate whether `ffprobe` against remote `video_url` values is actually the best way to get accurate duration data for all clips, or whether a simpler/more reliable approach exists — and only proceed with the ffprobe plan if it holds up. If it does: query every clip, probe durations, write `clip_durations.json`; then find candidate duplicate pairs (duration within 0.5s AND similar titles) and write `duplicate_candidates.md`, read-only, no clips or other files touched.

## Step-0 investigation: is ffprobe-per-clip the right call?

**Yes — investigated first, and it held up, with one real wrinkle worth flagging.** Checked the alternatives the task itself raised, plus one it didn't:

- **Duration already stored somewhere?** No. Grepped the whole codebase for `duration` — the only hits are `hover_events.duration_ms` (hover-preview analytics, unrelated) and a Tailwind `duration-300` class. [components/ClipsManager.jsx](components/ClipsManager.jsx)'s thumbnail-capture flow *does* read a video's native `duration` via the browser's `<video>` element during upload, but only transiently, off the local `File` object, to pick a thumbnail seek time — it's never written back to the database. There is no `duration` column on `clips`, and nothing populates one.
- **R2/Supabase Storage object metadata?** Not viable even in principle — S3-compatible object metadata (what R2 and Supabase Storage both expose) doesn't include container-level video duration unless something explicitly wrote it as custom metadata at upload time, and nothing in [components/ClipsManager.jsx](components/ClipsManager.jsx)'s upload flow does that. Getting duration fundamentally requires parsing each file's own container metadata, one way or another — there's no shortcut through storage-layer APIs.
- **Domain/scale check**: queried Supabase directly (read-only) — **541 clips** total (not 544), all with a `video_url`; 540 on `cdn.oldmantriesbjj.com` (R2 via Cloudflare) and exactly **1** still on `supabase.co` (a leftover from before the thumbnail-only R2 migration in `scripts/migrate-thumbnails-to-r2.js`, which never touched `video_url`).
- **CORS**: not a real concern as suspected — CORS only applies to browser `fetch`, not a server-side/CLI tool like `ffprobe`.
- **Rate limits / full downloads**: confirmed empirically rather than assumed. `HEAD` on a sample CDN URL showed `Accept-Ranges: bytes`, so `ffprobe` can read just the container metadata via HTTP range requests instead of downloading the whole file — verified directly: three sample probes completed in 0.3–3.3s each on files up to 16MB. R2 has no egress fees regardless, so even a full download wouldn't have been a cost concern; it just wasn't necessary.
- **The one real wrinkle**: `ffprobe` **was not installed** in this environment (checked — not on `PATH`, no `choco`/`scoop`). Rather than treating that as a blocker, installed the `@ffprobe-installer/ffprobe` npm package (a portable prebuilt binary, no admin rights needed) into a scratch directory outside the repo — this is the one deviation from the plan as literally written, and it's what made proceeding possible at all.

Conclusion: no simpler or more reliable approach exists — this really is a per-file container-parsing problem, not a metadata-lookup problem. Proceeded with the plan, with two adjustments: `ffprobe` obtained via the portable npm package rather than assumed pre-installed, and the probing script itself run with concurrency 10 (mirroring `scripts/migrate-thumbnails-to-r2.js`'s existing pattern) rather than strictly serial, to keep total wall-clock time down.

## What was done

Wrote a throwaway Node script (not committed — lives only in the session's scratch directory, consistent with "read-only analysis only") that: queries Supabase for `id, title, category, video_url` for all clips; runs `ffprobe -v error -show_entries format=duration -of csv=p=0 <video_url>` per clip via `child_process.execFile` with a 30s timeout, at concurrency 10; and writes results. **All 541/541 clips probed successfully in 24 seconds, zero failures** — including the one Supabase-Storage-hosted clip. Durations ranged 3.6s–368.71s. Wrote `clip_durations.json` at the repo root as an array of `{ id, title, category, duration_seconds }`, exactly the requested shape.

For duplicate candidates: compared all 146,070 clip pairs, flagging any pair with `|durationA - durationB| <= 0.5` **and** similar titles — normalized-text exact match, substring containment, or ≥50% overlap on significant words (reusing `lib/clipSort.js`'s exact `STOPWORDS` list, since these are the same BJJ-technique titles that list was already tuned for). Found **28 candidate pairs**, several with 0.00s duration difference and titles differing only by a trailing "2"/"(2)"-style suffix (e.g. "Cross Collar from back 2" vs. "Cross Collar from back (2)") — a strong, plausible duplicate-upload pattern already visible in this dataset. A few pairs are weaker matches (single generic shared words like "sweep"), included as-is per the instruction to keep this a raw candidate list for human judgment, not a filtered/opinionated one. Wrote `duplicate_candidates.md` at the repo root, one pair per section with both clip IDs, titles, durations, and categories — no commentary or recommendations included, per the request.

## Verification
Both output files were generated from a live, successful run against the real database and CDN (541/541 probes succeeded, 0 errors) — this is empirical data, not a build/compile check. There is no `npm run build` step relevant to this task (no application code was touched). Spot-checked the JSON and Markdown output shapes directly before committing.

## What was NOT verified
The duplicate-candidate heuristic is exactly what was asked for — "simple substring/fuzzy check," explicitly for human eyeballing — and was not validated against ground truth (no one confirmed any of the 28 pairs are actually the same underlying clip; several may be false positives, e.g. two genuinely different clips that happen to both be short generic "sweep" videos of similar length). The one Supabase-Storage-hosted clip's probe succeeded, but wasn't separately spot-checked for correctness beyond that. Nothing about actual video content (e.g. whether two same-duration clips are frame-for-frame identical vs. just coincidentally similar length) was inspected — duration and title are the only two signals in play, exactly as scoped in the task.

## Bottom line
ffprobe-per-clip against remote `video_url`s was confirmed to be the right (and only real) approach after checking storage-metadata alternatives, domain distribution, range-request support, and rate-limit/CORS concerns directly rather than assuming — the sole real gap was `ffprobe` not being pre-installed, resolved via a portable npm package rather than changing approach. All 541 clips were probed successfully with zero failures in 24 seconds. `clip_durations.json` (541 entries) and `duplicate_candidates.md` (28 candidate pairs, raw list for manual review) are committed at the repo root; no clips or other files were modified. Changes committed and pushed to `main` (commits `6e1cdb7` and `f3079bb`).
